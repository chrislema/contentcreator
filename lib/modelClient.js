...
// Lets ANY API model (OpenAI-compatible, GLM, Anthropic) use registered MCP
// tools via native function-calling. The app executes each tool call in-process
// through the MCP client and feeds the result back, looping until the model
// stops requesting tools. This is the non-CLI equivalent of the Claude CLI's
// --mcp-config tool access, so MCPs work regardless of which model is selected.
...
// Collect tool definitions across all connected MCPs plus a name -> mcp lookup.
async function gatherMcpTools(mcps) {
  const { connectMcp } = require('./mcpClient');
  const toolDefs = [];
  const toolToMcp = {};
  for (const mcp of mcps) {
    if (!mcp || !mcp.connected) continue;
    let tools = mcp.tools || [];
    // Refresh if we don't have input schemas (stored tools may predate them)
    if (!tools.length || !tools.every((t) => t.inputSchema)) {
      try {
    ...
    for (const t of tools) {
      if (toolToMcp[t.name]) continue; // first MCP wins on a name collision
      toolToMcp[t.name] = mcp;
      toolDefs.push(t);
    }
  }
  return { toolDefs, toolToMcp };
}

async function executeMcpTool(toolToMcp, name, argObj, transcript) {
  const { callTool } = require('./mcpClient');
  const mcp = toolToMcp[name];
  if (!mcp) return `Error: tool "${name}" is not available.`;
  try {
    transcript.push(`→ ${name}(${JSON.stringify(argObj)})`);
    const out = await callTool(mcp, name, argObj || {});
    return String(out).slice(0, MAX_TOOL_RESULT_CHARS);
  } catch (e) {
    const msg = `Error calling ${name}: ${e.message}`;
    transcript.push(msg);
    return msg;
  }
}

function joinTranscript(transcript, finalText) {
  const log = transcript.filter(Boolean).join('\n');
  if (!log) return finalText || '';
  return finalText ? `${log}\n\n${finalText}` : log;
}

// OpenAI-compatible (GPT, GLM, etc.) agentic loop
async function openAiToolLoop(config, messages, maxTokens, toolDefs, toolToMcp) {
  const base = config.baseUrl.replace(/\/$/, '');
  const hasVersion = /\/v\d+$/.test(base);
  ...
  for (let iter = 0; iter < MAX_TOOL_ITERS; iter++) {
    ...
    if (!toolCalls.length) return joinTranscript(transcript, msg.content || '');

    if (msg.content) transcript.push(msg.content);
    for (const tc of toolCalls) {
      let argObj = {};
      try { argObj = JSON.parse(tc.function?.arguments || '{}'); } catch {}
      const result = await executeMcpTool(toolToMcp, tc.function?.name, argObj, transcript);
      convo.push({ role: 'tool', tool_call_id: tc.id, content: result });
    }
  }
  return joinTranscript(transcript, '(Reached tool-call limit before finishing.)');
}

// Anthropic API agentic loop
async function anthropicToolLoop(config, messages, maxTokens, toolDefs, toolToMcp) {
  const url = `${config.baseUrl.replace(/\/$/, '')}/v1/messages`;
  const systemMsg = messages.find((m) => m.role === 'system');
  ...
  for (let iter = 0; iter < MAX_TOOL_ITERS; iter++) {
    ...
    if (!toolUses.length) return joinTranscript(transcript, textOut);

    if (textOut) transcript.push(textOut);
    const results = [];
    for (const tu of toolUses) {
      const result = await executeMcpTool(toolToMcp, tu.name, tu.input || {}, transcript);
...
// Run an agentic MCP tool loop for an API model (non-CLI).
async function callWithTools(config, messages, maxTokens, options) {
  const mcps = (options.mcps || []).filter((m) => m && m.connected);
  const { toolDefs, toolToMcp } = await gatherMcpTools(mcps);
  if (config.provider === 'anthropic') {
    return anthropicToolLoop(config, messages, maxTokens, toolDefs, toolToMcp);
  }
  return openAiToolLoop(config, messages, maxTokens, toolDefs, toolToMcp);
}
...
async function chatCompletion(modelConfig, messages, maxTokens, options = {}) {
  ...
  // API models (Anthropic, OpenAI-compatible, GLM): when a chat needs MCP tools,
  // run the in-app agentic loop so tools work regardless of which model is used.
  if (options.enableTools && options.mcps && options.mcps.length) {
    return callWithTools(modelConfig, cleanMessages, maxTokens, options);
  }
