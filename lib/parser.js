...
// Parse a markdown file into flat audience entries (one per micro-segment)
function parseAudienceMarkdown(text, filename) {
  // Extract document name from first H1 (used as source tag)
  const h1Match = text.match(/^#\s+(.+)$/m);
  const sourceName = h1Match
    ? h1Match[1].replace(/Audience Segment Profile:?/i, '').trim() || filename.replace(/\.md$/, '')
    : filename.replace(/\.md$/, '');
  ...
  for (let i = 0; i < segHeaders.length; i++) {
    ...
    if (descM) aud.description = descM[1].trim().slice(0, 1000);

    // Goal Pyramid
    const pyramidM = section.match(/Level 1[^:]*:\s*(.+?)(?:\n|$)/);
    if (pyramidM) aud.goalPyramid.level1 = pyramidM[1].trim();
    const pyramid2 = section.match(/Level 2[^:]*:\s*(.+?)(?:\n|$)/);
    if (pyramid2) aud.goalPyramid.level2 = pyramid2[1].trim();
    const pyramid3 = section.match(/Level 3[^:]*:\s*(.+?)(?:\n|$)/);
    if (pyramid3) aud.goalPyramid.level3 = pyramid3[1].trim();
    const pyramid4 = section.match(/Level 4[^:]*:\s*(.+?)(?:\n|$)/);
    if (pyramid4) aud.goalPyramid.level4 = pyramid4[1].trim();
    ...
    if (pushM) aud.fourForces.push = pushM[1].trim().slice(0, 50);
    const magM = section.match(/Magnetism[^:]*:\s*\*?\*?([^*\n]+)/i);
    if (magM) aud.fourForces.magnetism = magM[1].trim().slice(0, 50);
    const anxM = section.match(/Anxiety[^:]*:\s*\*?\*?([^*\n]+)/i);
    if (anxM) aud.fourForces.anxiety = anxM[1].trim().slice(0, 50);
    const habM = section.match(/Habit[^:]*:\s*\*?\*?([^*\n]+)/i);
    if (habM) aud.fourForces.habit = habM[1].trim().slice(0, 50);
    const netM = section.match(/Net Force[^:]*:\s*(.+?)(?=\n###|\n##|\n#)/is);
    if (netM) aud.fourForces.netForce = netM[1].trim().slice(0, 1000);
    ...
    if (hireM) {
      ...
      if (moments) {
        aud.hiringMoments = moments.map((mom) => mom.replace(/\*\*/g, '').trim());
      }
    }

    if (aud.description || aud.goalPyramid.level1) {
      audiences.push(aud);
    }
  }
